...
from adapters.types.sofascore_tournaments import UniqueTournamentResponse, UniqueTournamentSeasonsResponse, \
    UniqueTournamentSeasonRoundsResponse, UniqueTournamentSeasonEventsRoundResponse
...
class SofascoreTournamentRepository(TournamentRepository):
    ...
    async def get_tournament_round_events(self, tournament_id: int, season_id: int, round_number: int) -> list[Event]:
        try:
            response = await self.http_client.get(f"https://api.sofascore.com/api/v1/unique-tournament/{tournament_id}/season/{season_id}/events/round/{round_number}")
            if "error" in response:
                if response["error"].get("code") == 404:
                    raise TournamentNotFoundError(f"Tournament with ID {tournament_id} not found")
            response_data = cast(
    ...
    def _map_response_to_tournament_season_round_events_model(self, response_data: UniqueTournamentSeasonEventsRoundResponse) -> list[Event]:
        return [
            Event(
